  ...
  <section id="coach-athletes-cabinet" class="coach-cabinet">
    ...
    <div v-else class="row g-4">
      <div v-for="a in athletes" :key="a.id" class="col-12 col-xl-6">
        <div class="kn-athlete-card p-4 d-flex flex-column flex-md-row gap-4" :class="{ 'kn-athlete-card--accent': isGoldStandard(a.id) }">
          ...
          <div class="flex-grow-1 d-flex flex-column justify-content-between">
            ...
            <div class="mt-3">
              <div class="kn-track kn-coach-progress-track">
                <div class="kn-track__fill kn-track__fill--secondary kn-coach-progress-fill" :style="{ width: `${progressForAthlete(a.id)}%` }" />
              </div>
            </div>
            <div class="d-flex flex-wrap gap-2 mt-4">
              <button type="button" class="kn-coach-action kn-coach-action--primary" @click="emit('add-training', a.id)">
                <span class="material-symbols-outlined">add_task</span>
                {{ t('coach.addTraining') }}
              </button>
              <button type="button" class="kn-coach-action kn-coach-action--soft" @click="emit('view-progress', a.id)">
                <span class="material-symbols-outlined">analytics</span>
                {{ t('kn.viewProgress') }}
              </button>
              <button type="button" class="kn-coach-action kn-coach-action--ghost" @click="emit('ai-recommendation', a.id)">
